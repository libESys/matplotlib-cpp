...
public:
    ...
    virtual int init();
    virtual void init_sys_path();
    virtual void after_matplotlib_import();

    static void set(std::shared_ptr<Interpreter> interpreter);
    static std::shared_ptr<Interpreter> get();
    static std::shared_ptr<Interpreter> get_or_new();

    void set_root_path(const std::string &root_pyhton);
    const std::string &get_root_path() const;

    void add_sys_path(const std::string &path);
    void add_sys_path_rel_root(const std::string &rep_path);

    const std::vector<std::string> &get_sys_paths() const;
...
protected:
    static std::shared_ptr<Interpreter> s_interpreter;
    std::vector<std::string> m_sys_paths;
    std::string m_root_path;
    int m_argc = 0;
    char **m_argv = nullptr;
};

std::shared_ptr<Interpreter> Interpreter::s_interpreter;

void Interpreter::set(std::shared_ptr<Interpreter> interpreter)
{
    s_interpreter = interpreter;
}

std::shared_ptr<Interpreter> Interpreter::get()
{
    return s_interpreter;
}

std::shared_ptr<Interpreter> Interpreter::get_or_new()
{
...
{
    boost::filesystem::path tcl_lib = get_root_path();
    if (!tcl_lib.empty())
    {
        tcl_lib /= "tcl";
        tcl_lib /= "tcl8.6";
...
#ifdef WIN32
        _putenv_s("TCL_LIBRARY", tcl_lib.string().c_str());
#else
        setenv("TCL_LIBRARY", tcl_lib.string().c_str(), 1);
#endif
    }
...
void Interpreter::init_sys_path()
{
    std::string cmd;
    ...
    for (auto &sys_path : get_sys_paths())
    {
        if (!boost::filesystem::exists(sys_path)) continue;

        std::string py_str = boost::algorithm::replace_all_copy(sys_path, "\\", "\\\\");
        cmd = "sys.path.append(\"" + py_str + "\")";
        PyRun_SimpleString(cmd.c_str());
    }
}

void Interpreter::after_matplotlib_import()
{
}

void Interpreter::add_sys_path(const std::string &path)
{
    m_sys_paths.push_back(path);
}

void Interpreter::add_sys_path_rel_root(const std::string &rel_path)
{
    assert(!get_root_path().empty());
    ...
    add_sys_path(path.make_preferred().string());
}

const std::vector<std::string> &Interpreter::get_sys_paths() const
{
    return m_sys_paths;
}

void Interpreter::set_root_path(const std::string &root_path)
{
    m_root_path = root_path;
}

const std::string &Interpreter::get_root_path() const
{
    return m_root_path;
}

void Interpreter::set_args(int argc, char **argv)
{
    m_argc = argc;
    m_argv = argv;
}

int Interpreter::get_argc()
{
    return m_argc;
}

char **Interpreter::get_argv()
{
    return m_argv;
}
...
private:
    ...
    _interpreter()
    {
        std::shared_ptr<Interpreter> interpreter = Interpreter::get_or_new();

        interpreter->init();

        interpreter->init_sys_path();
        ...
        interpreter->after_matplotlib_import();
    }
